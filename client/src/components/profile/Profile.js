...
const Profile = ({ match, getProfileByID, profile: { profile, loading}, auth }) => {
    ...
    return (
        <Fragment>
            {profile === null || loading ? <Spinner /> : (<Fragment><Link to='/profiles' className='btn btn-light'>Back to Profiles</Link>{auth.isAuthenticated && auth.loading === false && auth.user._id === profile.user._id && (<Link to='/profile-form' className='btn btn-dark'>Edit Profile</Link>) }
